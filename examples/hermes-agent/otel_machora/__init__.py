"""otel_machora — Hermes plugin for Machora-native OTLP observability.

Traces Hermes sessions, turns, LLM calls, tool calls, and subagents to a
Machora OTLP/HTTP traces endpoint using the **machora.\*** semantic keys
(see packages/shared/src/otel/semantics/machora.ts in the Machora repo).

The exported span tree follows the machora.span.kind vocabulary so Machora
classifies spans with its native adapter (highest priority):
...
Attributes use machora keys: machora.input/output (JSON), machora.model.name,
machora.token.input/output, machora.tool.name, machora.tool.call.id,
machora.session.id, machora.user.id, machora.agent.name. ``machora.span.kind``
is stored directly as ``observation.type``.
...
observability/otel_machora``). At runtime the plugin also requires the
...
Env vars (set via ``hermes tools`` or ~/.hermes/.env):
  ...
  HERMES_OTEL_MACHORA_MAX_CHARS - max chars per input/output value
      (default: 12000)
  HERMES_OTEL_MACHORA_DEBUG - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("HERMES_OTEL_MACHORA_DEBUG")


def _debug(message: str) -> None:
    if _debug_enabled():
        logger.info("Machora OTLP: %s", message)


def _max_chars() -> int:
    raw = _env("HERMES_OTEL_MACHORA_MAX_CHARS", "12000")
    try:
...
def _get_provider():
    ...
    endpoint = _env("HERMES_OTEL_MACHORA_ENDPOINT") or _DEFAULT_ENDPOINT
    headers = _parse_headers(_env("HERMES_OTEL_MACHORA_HEADERS"))
    service_name = _env("HERMES_OTEL_MACHORA_SERVICE_NAME", "hermes")
    ...
    try:
        ...
        logger.warning(
            "Machora OTLP plugin: provider init failed, tracing disabled: %s",
            exc,
        )
        _PROVIDER = _INIT_FAILED
        return None
...
def _start_span(provider, name: str, kind: str, parent_span: Any = None,
    ...
    tracer = provider.get_tracer("hermes.otel_machora")
    ...
    span.set_attribute("machora.span.kind", kind)
    return span
...
def on_session_start(*, session_id: str = "", user_id: str = "",
    ...
    with _LOCK:
        ...
        attributes: dict[str, Any] = {"machora.session.id": session_id}
        if user_id:
            attributes["machora.user.id"] = user_id
        if agent_name:
            attributes["machora.agent.name"] = agent_name
...
def pre_llm_call(*, session_id: str = "", turn_id: str = "", model: str = "",
    ...
    if turn_id:
        attributes["machora.turn_id"] = turn_id
    if model:
        attributes["machora.model.name"] = model
    with _LOCK:
        _end_span(state.turn_span)
...
def pre_api_request(*, task_id: str = "", session_id: str = "", model: str = "",
    ...
    if model:
        attributes["machora.model.name"] = model
    if provider:
        attributes["machora.provider"] = provider
    # machora.input：优先存消息数组（前端按 role 渲染），
    ...
    if req_value:
        attributes["machora.input"] = req_value
...
def post_api_request(*, session_id: str = "", model: str = "", response: Any = None,
    ...
    span, _started = entry
    if model:
        span.set_attribute("machora.model.name", model)
    # machora.output：优先存 assistant 消息数组（前端按 role 渲染），
    ...
    if out_value:
        span.set_attribute("machora.output", out_value)
    if finish_reason:
        span.set_attribute("machora.finish_reason", finish_reason)
    input_tokens, output_tokens = _usage_tokens(usage)
    if input_tokens is not None:
        span.set_attribute("machora.token.input", input_tokens)
    if output_tokens is not None:
        span.set_attribute("machora.token.output", output_tokens)
    if api_duration and api_duration > 0:
        span.set_attribute("machora.duration_s", round(api_duration, 3))
    _end_span(span)
    state.last_updated_at = time.time()
...
def api_request_error(*, session_id: str = "", api_call_count: int = 0,
    ...
    span.set_attribute("machora.level", "ERROR")
    if reason:
        span.set_attribute("machora.error.reason", reason)
    if error:
        if isinstance(error, dict):
            for k, v in error.items():
                if k in ("type", "message") and isinstance(v, (str, int, float, bool)):
                    span.set_attribute(f"machora.error.{k}", v)
        else:
            span.set_attribute("machora.error.message", _truncate(str(error)))
    if status_code is not None:
        try:
            span.set_attribute("machora.error.status_code", int(status_code))
        except (TypeError, ValueError):
            pass
    _end_span(span)
    state.last_updated_at = time.time()
...
def pre_tool_call(*, tool_name: str = "", args: Any = None, session_id: str = "",
    ...
    attributes: dict[str, Any] = {}
    if tool_name:
        attributes["machora.tool.name"] = tool_name
    if tool_call_id:
        attributes["machora.tool.call.id"] = tool_call_id
    args_value = _json_value(args)
    if args_value:
        attributes["machora.input"] = args_value
    with _LOCK:
        parent = state.turn_span if state.turn_span is not None else state.root_span
        span = _start_span(
            provider_obj,
...
def post_tool_call(*, tool_name: str = "", result: Any = None, session_id: str = "",
    ...
    if result is not None:
        ...
        if out_value:
            span.set_attribute("machora.output", out_value)
    if status:
        span.set_attribute("machora.tool.status", status)
        if status in ("error", "blocked", "cancelled"):
            span.set_status(_STATUS_ERROR)
            span.set_attribute("machora.level", "ERROR")
    if duration_ms is not None:
        try:
            span.set_attribute("machora.duration_ms", int(duration_ms))
        except (TypeError, ValueError):
            pass
    _end_span(span)
    state.last_updated_at = time.time()
...
def subagent_start(*, parent_session_id: str = "", child_session_id: str = "",
    ...
    attributes: dict[str, Any] = {}
    if child_session_id:
        attributes["machora.session.id"] = child_session_id
    if child_role:
        attributes["machora.agent.name"] = child_role
    if child_goal:
        attributes["machora.goal"] = _truncate(child_goal)
    with _LOCK:
        span = _start_span(
            provider_obj,
...
def subagent_stop(*, parent_session_id: str = "", child_session_id: str = "",
    ...
    if child_summary is not None:
        ...
        if out_value:
            span.set_attribute("machora.output", out_value)
    if status:
        span.set_attribute("machora.agent.status", status)
    if duration_ms is not None:
        try:
            span.set_attribute("machora.duration_ms", int(duration_ms))
        except (TypeError, ValueError):
...
def register(ctx) -> None:
    if _env_bool("HERMES_OTEL_MACHORA_DISABLED"):
        return
