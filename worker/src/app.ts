...
export function registerQueueProcessors(): void {
  queueBus.consume<IngestionQueuePayload>(QUEUES.ingestion, async (payload) => {
    // 在线自动评估：ingestion 完成后，对启用的评估配置自动创建任务（AgentLoop 在线评估模式）
    await triggerOnlineEvaluations(payload.traceId);
  });
...
/**
 * 在线自动评估：查询 enabled 的评估配置，为当前 trace 逐条创建评估任务并入队。
 * 防重复：同 trace 同 name 已存在未终结（PENDING/RUNNING）任务时跳过。
 */
async function triggerOnlineEvaluations(traceId: string): Promise<void> {
  ...
  console.log(
    `[ingestion] trace=${traceId} online-evals=${configs.length}`,
  );
}
