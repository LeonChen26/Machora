...
function buildQuery(agent: string, days: number): string {
  const params = new URLSearchParams();
  params.set("name", agent);
  ...
  return (
    <>
      <div className="breadcrumb">
        ...
        <span className="mono muted">agents</span>
      </div>

      <div className="page-head">
        <div>
          <h1>
            <span className="badge green" style={{ fontSize: 16 }}>{agent}</span>
          </h1>
          <div className="sub">
      ...
      <div className="seg">
        {DAY_OPTIONS.map((d) => (
          <Link
            key={d}
            href={`/analytics/agents?${buildQuery(agent, d)}`}
            prefetch={false}
